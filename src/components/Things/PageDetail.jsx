      ...
      {showAddModal && (
        <div className="fixed top-0 left-0 w-full h-full bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96 max-h-[80vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">Chọn thiết bị để thêm</h3>
            <ul>
              {availableDevices.length === 0 && (
                <p className="text-gray-500 italic">Không còn thiết bị nào để thêm.</p>
              )}
              {availableDevices.map(device => (
                <li key={device.uid} className="flex justify-between items-center bg-green-50 p-2 rounded mb-2">
                  <span>{device.name}</span>
            ...
            <div className="mt-4 flex justify-end">
              <button onClick={() => setShowAddModal(false)} className="text-gray-600 hover:text-black">
                Đóng
              </button>
            </div>
          </div>
