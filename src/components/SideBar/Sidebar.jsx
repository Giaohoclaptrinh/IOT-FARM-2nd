...
const Sidebar = () => {
    ...
    return (
       (auth.currentUser) ? (<aside className="fixed min-h-screen min-w-60  max-w-60  overflow-auto text-gray-700 border-r border-gray-300 x top-0  left-0 bg-[#ffffff] ">
            <div className="px-2 relative">
                <div className={`flex mt-4 p-2 relative hover:bg-blue-100 hover:rounded-lg cursor-pointer
                ${userDown && ('bg-blue-100 rounded-lg')}`}
                onClick={(prev)=>{
                    setUserDown(!userDown)
                }}>
                   <span className="inline-flex  rounded-full items-center justify-center 
                   w-12 h-12  bg-blue-400 text-white  font-primary" > {userCurrent.userName.split("").slice(0,2)}</span>
                   <div className="ml-4">
                ...
                <div className={`
                    w-min-full  absolute top-[75px] left-2 z-10 shadow-lg bg-white  transition-all duration-300 ease-out overflow-hidden ${
        userDown ? 'h-[146px] opacity-100' : 'h-0 opacity-0 pointer-events-none'
                     } `}>
                    {
                        menuList.map((item)=>{
                            return (
                               (item.title !== "SignOut") ? (<Link className={
                                    `
                                    flex items-center  space-y-2 border-b px-2 py-2
                                hover:bg-gray-300 hover:bg-bgMain 
                                pr-14
